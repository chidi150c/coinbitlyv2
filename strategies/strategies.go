		...
		go func() { //Goroutine to store TS into Database
			...
			for {
				...
				case ts.UpgdChan <- true:
					...
					fmt.Printf("after update via chan ts.ID: %d",ts.ID)
	...
	case "Buy":
		...
		if ts.QuoteBalance < totalCost {
			if (!ts.InTrade) && ts.StopLossTrigered {
				...
				ts.Log.Printf("With vector swapped between last and 2nd to last !!!")
			} else {
				ts.InTrade = true
